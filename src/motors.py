...
## I edit this file to control the DC motros 
...
from base_control.msg import car_speed_way
...
def CommandCallback(commandMessage):
  speed = 0.3
  linear = commandMessage.linear
  angular = commandMessage.angular
  if linear > 0 :
    angular = 0
    forwards(speed)
    time.sleep(linear*0.4)
    coast()
  elif linear < 0:
    angular = 0
    backwards(speed)
    time.sleep(abs(linear)*0.4)
    coast()
  elif (linear == 0) and (angular > 0):  ##angular speed
    turn(-speed)
    time.sleep(abs(angular)*0.1)
    coast()
  elif (linear == 0) and (angular < 0):  ##angular speed
    turn(speed)
    time.sleep(abs(angular)*0.1)
...
def wheel_control():
  ...
  rospy.Subscriber('wheel_command', car_speed_way, CommandCallback)
  rospy.spin()
